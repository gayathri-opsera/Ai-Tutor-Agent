-- Seed: default roles with fixed UUIDs
-- Clean up any auto-generated rows first so our fixed UUIDs are used consistently.
DELETE FROM user_roles WHERE role_id IN (
  SELECT id FROM roles WHERE name IN ('Learner','Creator','Admin','SuperAdmin')
);
DELETE FROM roles WHERE name IN ('Learner','Creator','Admin','SuperAdmin');
